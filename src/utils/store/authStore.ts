...
interface StoreState {
  ...
  forgetPassowrd: (number: number) => Promise<boolean>;
...
const useAuthStore = create<StoreState>()((set, get) => ({
  ...
  forgetPassowrd: async number => {
    set({isLoading: true, error: null});
    try {
      await axios.post(`${apiLink}/request-password-reset`, {mobile: number});
      Alert.alert('OTP sent successfully');
  ...
  initializeUser: async () => {
    ...
    if (token && user) {
      set({token, user: JSON.parse(user), fcmToken, deviceId});
    }
  },
